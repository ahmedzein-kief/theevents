...
class Validator {
  ...
  static String? vendorEmail(String? value) {
    if (value == null || value.isEmpty) {
      return VendorAppStrings.errorEmailRequired;
    }
    String pattern = r'^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$';

    RegExp regExp = new RegExp(pattern);
    if (!regExp.hasMatch(value)) {
      return VendorAppStrings.errorValidEmail;
    }
    return null;
  }
  ...
  static String? fieldCannotBeEmpty(dynamic? value) {
    if (value == null || value.isEmpty) {
