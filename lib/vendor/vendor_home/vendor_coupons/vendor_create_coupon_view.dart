...
class _VendorCreateCouponViewState extends State<VendorCreateCouponView> with MediaQueryMixin {
  // Form key
  ...
  final TextEditingController _numberOfCouponsController = TextEditingController();
  ...
  void initState() {
    ...
    String formattedDate = DateFormat('yyyy-MM-dd').format(now); // e.g., "2025/01/27"
    String formattedTime = DateFormat('HH:mm').format(now); // e.g., "14:30"
  ...
  Future<void> _generateCouponCode(VendorGenerateCouponCodeViewModel provider) async {
    try {
      setProcessing(true);
      await provider.vendorGenerateCouponCode();
      if (provider.apiResponse.status == ApiStatus.COMPLETED) {
        _couponCodeController.text = provider.apiResponse.data?.data.toString() ?? '';
      }
      setProcessing(false);
    } catch (e) {
      setProcessing(false);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: VendorCommonAppBar(title: "Coupons"),
      body: Utils.modelProgressHud(processing: _isProcessing, child: _buildUi(context)),
              ...
              SimpleCard(
                ...
                children: [
                  ...
                  CustomCheckboxWithTitle(
                    ...
                    subTitle: "The list of coupon codes will be displayed at the checkout page and customers can choose to apply.",
                    onChanged: (value) {
                      setState(() {
                  ...
                  CustomDropdown(
                      ...
                      onChanged: (value) {
                        setCouponType(value);
                        _couponTypeController.text = CouponViewUtils.getTypeOption(couponType);
                      }),
                  kFormFieldSpace,
                  ...
                  CustomTextFormField(
                      ...
                      hintText: "",
                      borderRadius: 4,
                      prefix: CouponViewUtils.discountFieldPrefixAndSuffix(couponType: couponType, isPrefix: true, screenWidth: screenWidth),
                      suffix: CouponViewUtils.discountFieldPrefixAndSuffix(couponType: couponType, isPrefix: false, screenWidth: screenWidth),
                    ...
                    children: [
                      ...
                      Padding(
                        ...
                        child: Column(
                          children: [
                            /// Start Date and time
                            fieldTitle(text: "Start Date", textStyle: CouponViewUtils.couponLabelTextStyle()),
                            kSmallSpace,
                            ...
                            // End Date and time
                            fieldTitle(text: "End Date", textStyle: CouponViewUtils.couponLabelTextStyle()),
                            kSmallSpace,
                            CustomDateTimePickerField(
                              ...
                              onTimeTap: () => showCupertinoDateTimePicker(
                                  ...
                                  onDateTimeChanged: (time) {
                                    setState(() {
                                      _endTimeController.text = DateFormat("HH:mm").format(time);
                                    });
                                  }),
                            ),
                        ...
                        child: Consumer<VendorCreateCouponViewModel>(
                          builder: (context, provider, _) {
                            return CustomAppButton(
                                ...
                                padding: EdgeInsets.symmetric(horizontal: screenWidth / 20, vertical: 5),
                                isLoading: provider.apiResponse.status == ApiStatus.LOADING,
                                onTap: () async {
                                  ...
                                  try {
                                    ...
                                    if (_formKey.currentState?.validate() ?? false) {
                                      _creteForm();
                                      await provider.vendorCreateCoupon(form: form, context: context);
                                      // setProcessing(false);

                                      /// Calling the get vendor coupons api
                                      final getCouponsProvider = Provider.of<VendorGetCouponsViewModel>(context, listen: false);
