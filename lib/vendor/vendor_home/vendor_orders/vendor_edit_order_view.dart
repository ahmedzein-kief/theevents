...
class _VendorEditOrderViewState extends State<VendorEditOrderView> with MediaQueryMixin {
  ...
  void _initializeStepper({required VendorGetOrderDetailsViewModel provider}) {
    ...
    stepperData = history
        .map((element) => StepperData(
            title: StepperText(element.historyVariables?.toString() ?? '', textStyle: detailsTitleStyle),
            subtitle: StepperText(element.createdAt?.toString() ?? '', textStyle: detailsDescriptionStyle),
            iconWidget: Container(
              decoration: const BoxDecoration(color: AppColors.lightCoral, borderRadius: BorderRadius.all(Radius.circular(30))),
              child: kShowVoid,
            )))
        .toList();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: VendorCommonAppBar(title: "Orders"),
  ...
  Widget _buildUi(BuildContext context) {
    return Consumer<VendorGetOrderDetailsViewModel>(builder: (context, provider, _) {
      /// current api status
  ...
  Widget _orderDetails({required context, required VendorGetOrderDetailsViewModel provider}) {
    ...
    return SimpleCard(
        ...
        expandedContent: Column(
          ...
          children: [
                ...
                children: [
                  ...
                  Expanded(
                    child: RichText(
                      ...
                      TextSpan(text: "Order Information ", style: TextStyle(color: Colors.black)),
                      TextSpan(text: orderData?.code.toString() ?? '--', style: TextStyle(color: Colors.black, fontWeight: FontWeight.w500))
                    ])),
                  ),
                  ...
                  if (orderData?.orderCompleted ?? false)
                    CustomIconButtonWithText(
                        ...
                        color: AppColors.getOrderStatusColor(orderData?.orderStatus?.value),
                  ...
                  children: [
                    Padding(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(item?.name?.toString() ?? '--', style: TextStyle(color: AppColors.lightCoral, fontSize: 17)),
                          kMinorSpace,
                          ...
                          RichText(
                            text: TextSpan(children: [
                              ...
                              TextSpan(text: "SKU: ", style: TextStyle(color: Colors.black, fontSize: 13)),
                              TextSpan(
                            ...
                            RichText(
                              text: TextSpan(children: [
                                TextSpan(
                                  ...
                                  style: TextStyle(color: AppColors.stoneGray, fontSize: 12),
                                ),
                                TextSpan(
                                  text: "Completed",
                                  style: TextStyle(color: AppColors.stoneGray, fontSize: 12),
                            ...
                            children: [
                              RichText(
                                text: TextSpan(
                                  ...
                                  children: [
                                    TextSpan(text: item?.priceFormat?.toString() ?? ''),
                                    WidgetSpan(
                                    ...
                                    TextSpan(text: item?.totalAmountFormat?.toString()),
                                  ],
                                ),
                              ),
                            ],
            ...
            Table(
              ...
              children: [
                ...
                TableRow(
                  ...
                  children: [
                    _buildCell(text: "Paid Amount", isEndAligned: true, isBold: true),

                    /// show full amount here if payment status is completed
                    _buildCell(text: orderData?.paymentStatus?.value == PaymentStatusConst.COMPLETED ? orderData?.amountFormat?.toString() ?? '--' : '--', isBold: true),
                  ],
                ),
              ],
            ),
                ...
                children: [
                  Consumer<VendorGenerateOrderInvoiceViewModel>(
                    builder: (context, generateInvoiceProvider, _) {
                      return CustomIconButtonWithText(
                          ...
                          onPressed: () async {
                            await generateInvoiceProvider.vendorGenerateOrderInvoice(orderId: widget.orderID.toString());
                          });
                    },
                  ),
                ],
                    ...
                    child: Consumer<VendorUpdateOrderViewModel>(
                      builder: (context, vendorUpdateOrderProvider, _) {
                        return CustomAppButton(
                          ...
                          padding: EdgeInsets.symmetric(horizontal: kPadding, vertical: kExtraSmallPadding),
                          isLoading: vendorUpdateOrderProvider.apiResponse.status == ApiStatus.LOADING,
                          onTap: () async {
                            try {
                              if (_noteController.text.isEmpty) {
                                AlertServices.showErrorSnackBar(message: ("Please add note."), context: context);
                              } else {
                                await vendorUpdateOrderProvider.vendorUpdateOrder(orderID: widget.orderID.toString(), description: _noteController.text, context: context);
                              }
                            } catch (e) {
                            }
                          },
              ...
              child: Column(
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      StatusLabel(
                        label: showOrderConfirmationLabel(!(orderData?.isConfirmed ?? false)),
                        icon: CupertinoIcons.checkmark_alt,
                        iconColor: !(orderData?.isConfirmed ?? false) ? AppColors.stoneGray : AppColors.success, // Pass the color you need
                      ),
                          ...
                          child: Consumer<VendorConfirmOrderViewModel>(
                            builder: (context, vendorConfirmOrderProvider, _) {
                              return CustomAppButton(
                                ...
                                onTap: () async {
                                  try {
                                    final result = await vendorConfirmOrderProvider.vendorConfirmOrder(orderID: widget.orderID.toString(), context: context);
                                    if (result) {
                                      await _onRefresh();
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  kExtraSmallSpace,
                  showStatusBox(statusText: shipment?.status?.label ?? '', color: AppColors.getShipmentStatusColor(shipment?.status?.value)),
                  ...
                  kExtraSmallSpace,
                  Text(Utils.formatTimestamp(shipment?.updatedAt?.toString() ?? '')),
                ],
              ),
            ),
            kSmallSpace,
            ...
            if (orderData != null && (orderData.shippingEnabled ?? false) || isShipmentStatusValid(shipment?.status?.value))
              Container(
                padding: EdgeInsets.symmetric(horizontal: kPadding, vertical: kSmallPadding),
                decoration: BoxDecoration(color: Colors.grey.shade200, border: Border(top: BorderSide(color: Colors.grey.shade300))),
                child: Row(
                  children: [
                    CustomIconButtonWithText(
                                  ...
                                  content: Column(
                                    ...
                                    children: [
                                      ...
                                      CustomDropdown(
                                        ...
                                        textStyle: TextStyle(color: Colors.black),
                                        menuItemsList: provider.apiResponse.data?.data?.shippingStatuses?.map((element) {
                                            ...
                                            onTap: () async {
                                              try {
                                                if (_shipmentStatusController.text.isEmpty) {
                                                  AlertServices.showErrorSnackBar(message: ("Please Select shipment status"), context: context);
                                                } else {
                                                  bool result = await updateStatusProvider.vendorUpdateShipmentStatus(
      ...
      children: [
        SimpleCard(
            ...
            expandedContent: Column(
              ...
              children: [
                Padding(
                  padding: EdgeInsets.only(top: kPadding, left: kPadding, right: kPadding, bottom: kExtraSmallPadding),
                  child: Text(
                    "History",
                ...
                /// Resend email button
                if (history.isNotEmpty == true && history.any((element) => element?.action?.toString() == "send_order_confirmation_email") == true)
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        ChangeNotifierProvider(
                            ...
                            child: Consumer<VendorSendConfirmationEmailViewModel>(builder: (context, sendConfirmationEmailProvider, _) {
                              return CustomAppButton(
                                buttonText: "Resend Email",
                                buttonColor: Colors.transparent,
                                textStyle: TextStyle(color: AppColors.lightCoral),
                                borderRadius: kSmallButtonRadius,
                                ...
                                onTap: () async {
                                  try {
                                    final bool result = await sendConfirmationEmailProvider.vendorSendConfirmationEmail(orderID: widget.orderID.toString(), context: context);
                                    if (result) {
                                      await _onRefresh();
                                    }
                                  } catch (e) {
  ...
  /// custom details
  Widget _customerInformation({required VendorGetOrderDetailsViewModel provider}) {
    final orderData = provider.apiResponse.data?.data;
        ...
        expandedContent: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Padding(
                  padding: EdgeInsets.only(left: kPadding, right: kPadding, top: kPadding, bottom: kSmallPadding),
              ...
              child: Column(
                ...
                children: [
                  ...
                  CircleAvatar(
                    ...
                    backgroundImage: customer?.avatarUrl != null && customer!.avatarUrl!.isNotEmpty ? NetworkImage(customer.avatarUrl!) : null,
                    onBackgroundImageError: (_, __) {},
                    // Handles image errors gracefully
                    child: (customer?.avatarUrl == null || customer!.avatarUrl!.isEmpty)
                        ? Icon(Icons.person, color: Colors.grey, size: 30) // Placeholder icon
                        : null,
                  ),

                  kExtraSmallSpace,
                  Text(customer?.name?.toString() ?? '', style: TextStyle(fontSize: 15, fontWeight: FontWeight.normal)),
                  kMinorSpace,
                  // number of orders
                  Row(
                    children: [Icon(Icons.image_aspect_ratio_sharp, size: 14), kExtraSmallSpace, Text(customer?.totalOrders?.toString() ?? ''), kExtraSmallSpace, Text("Order(s)")],
                  ),
                  kMinorSpace,
            ...
            Padding(
              padding: EdgeInsets.only(left: kPadding, right: kPadding, top: kSmallPadding, bottom: 0),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(
                          child: Text(
                        "Shipping Information",
                        style: detailsTitleStyle.copyWith(fontSize: 17, fontWeight: FontWeight.w500),
                      )),
                  ...
                  /// name
                  Text(shipping?.name?.toString() ?? '', style: TextStyle(fontSize: 15, fontWeight: FontWeight.normal)),
                  kMinorSpace,
                    ...
                    children: [
                      ...
                      Expanded(
                        child: GestureDetector(
                            onTap: () async {
                              await Utils.makePhoneCall(phoneNumber: shipping?.phone?.toString() ?? '', context: context);
                            },
                            child: Text(
            ...
            if (orderData?.cancelEnabled ?? false)
              Container(
                ...
                padding: EdgeInsets.only(left: kPadding, right: kPadding, top: kExtraSmallPadding, bottom: kExtraSmallPadding),
                            ...
                            onTap: () async {
                              deleteItemAlertDialog(
                                  ...
                                  onDelete: () async {
                                    try {
                                      Navigator.of(context).pop();
                                      final result = await cancelOrderProvider.vendorCancelOrder(orderID: widget.orderID.toString(), context: context);
                                      if (result) {
                                        await _onRefresh();
                                      }
                                    } catch (e) {
...
Widget _buildCell({Widget? customText, String? text, bool isBold = false, bool isEndAligned = false}) {
  return Padding(
...
TextStyle titleTextStyle() {
  return detailsTitleStyle.copyWith(fontSize: 16, fontWeight: FontWeight.w600);
}
...
bool isShipmentStatusValid(String? status) {
  const validStatuses = [ShipmentStatusConst.PENDING, ShipmentStatusConst.APPROVED, ShipmentStatusConst.ARRANGE_SHIPMENT, ShipmentStatusConst.READY_TO_BE_SHIPPED_OUT];
...
String showOrderConfirmationLabel(bool isConfirmed) {
  return !isConfirmed ? OrderConfirmationConst.ORDER_WAS_CONFIRMED : OrderConfirmationConst.CONFIRM_ORDER;
}
