...
class VendorCreatePackageViewState extends State<VendorCreatePackageView> with MediaQueryMixin {
  ///controllers
  ...
  void populateViewData([NewProductViewDataResponse? data]) {
    ...
    // set slug id
    _slugID = createProductPostData.slugId = productViewData?.slugId?.toString() ?? '';
    ...
    selectedPackageProducts = productViewData?.relatedProducts?.map((relatedProduct) {
  ...
  Future<bool> _createUpdatePackage() async {
    try {
      ...
      /// update package
      if (_currentPackageId != null && (_currentPackageId?.isNotEmpty == true)) {
        final result = await provider.vendorUpdatePackage(context: context, packageID: widget.packageID ?? '', productPostDataModel: createProductPostData);
        if (result) {
          await _onRefresh();
        }
      } else {
        final result = await provider.vendorCreatePackage(context: context, productPostDataModel: createProductPostData);
        if (result) {
  ...
  void _updatePackageProductsCount() {
    if (selectedPackageProducts.isNotEmpty) {
      _relatedProductsCountController.text = selectedPackageProducts.isNotEmpty ? "${selectedPackageProducts.length} related product(s) selected" : "No related products selected";
    } else {
  ...
  void _openPackageProductSearchScreen() async {
  ...
  _handlePackageProductsData(List<SearchProductRecord> packageProducts) {
  ...
  _handleOverViewData(result) {
    ...
    createProductPostData.sku = overviewModel?.sku ?? context.read<VendorGetPackageGeneralSettingsViewModel>().generalSettingsApiResponse.data?.data?.sku?.toString() ?? '';
...
// Add optional keywords only if they are not null or empty
  ...
  Future _createSlug({required String slug}) async {
    ...
    try {
      final slugGenerated = await createProductSlugProvider.vendorCreateProductSlug(productName: slug, productID: widget.packageID, slugID: _slugID, context: context);
  ...
  Widget _buildUi({required ThemeData theme, required BuildContext context}) {
    return Consumer<VendorGetPackageGeneralSettingsViewModel>(
      builder: (context, provider, _) {
        ...
        return Padding(
          ...
          child: ListView.separated(
            itemCount: provider.generalSettingsApiResponse.status == ApiStatus.ERROR ? 1 : createItems.length,
  ...
  /// General Information
  Widget _generalInformation({required ThemeData theme, required BuildContext context, required VendorGetPackageGeneralSettingsViewModel provider}) {
    final settings = provider.generalSettingsApiResponse.data?.data;
  ...
  /// overview section
  Widget _overviewSection({required ThemeData theme, required BuildContext context, required VendorGetPackageGeneralSettingsViewModel provider}) {
    return Column(
      children: [
        TitleWithArrow(
          title: "Overview",
  ...
  /// package products
  Widget _packageProducts({required ThemeData theme, required BuildContext context}) {
    return Column(
      children: [
        TitleWithArrow(
          title: "Package Products",
