...
class _CrossSellingProductsSearchScreenState extends State<CrossSellingProductsSearchScreen> {
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  _buildFormattedText(
                    title: "Price field",
                    description: "Enter the amount you want to reduce from the original price. Example: If the original price is \$100, enter 20 to reduce the price to \$80.",
                  ),
                  SizedBox(height: 8),
                  _buildFormattedText(
                    title: "Type field",
                    description: "Choose the discount type: Fixed (reduce a specific amount) or Percent (reduce by a percentage).",
                  ),
                ],
              ),
            ),
        ...
        children: [
          SpinnerRecordListTile(
            ...
            bottomWidget: Padding(
              padding: EdgeInsets.only(left: 8, right: 8, bottom: 12), // Adds padding on both sides
              child: Row(
                children: [
                  Expanded(
                    flex: 1,
                    child: CustomTextFormField(
                      ...
                      onChanged: (value) {
                        setState(() {
                          print("User Input 1 : $value"); // Debugging user input
                          int index = listSearchProducts.indexWhere((option) => option.id == product.id);
                          if (index != -1) {
                            print("User Input 2 : $value"); // Debugging user input

                            if (value is String) listSearchProducts[index].price = value.isNotEmpty ? int.parse(value) : 0;
                          }
                        });
                      },
                      required: false,
                  ...
                  SizedBox(width: 8), // Adds spacing between input field and dropdown
                  Expanded(
  ...
  Widget _buildFormattedText({required String title, required String description}) {
    return RichText(
      text: TextSpan(
