...
class VendorEditVariations extends StatefulWidget {
  ...
  VendorEditVariations({super.key, this.productVariationsID, required this.productID});
...
class _VendorEditVariationsState extends State<VendorEditVariations> with MediaQueryMixin {
  ...
  Map<String, String> _finalAttributesDataForCreateVariation = {};
  ...
  Future _onRefresh() async {
    try {
      ...
      if (widget.productVariationsID == null && widget.productID.isNotEmpty) {
        ...
        await provider.getEditVariations(context, widget.productVariationsID ?? '');
        ...
        if (provider.editVariationsApiResponse.status == ApiStatus.COMPLETED) {
          ...
          overviewModel = VendorProductOverviewModel(
            ...
            chooseDiscountPeriod: ((overview?.startDate?.toString().isNotEmpty ?? false) && (overview?.endDate?.toString().isNotEmpty ?? false)),
  ...
  Future _createUpdateVariation() async {
    try {
      ...
      variationPostDataModel.addedAttributes = {
        for (var attr in listAttributes) '${attr.id}': attr.attributes.isNotEmpty ? (attr.attributes.firstWhere((childAttr) => childAttr.isDefault == 1).id?.toString() ?? '') : '',
      };
      ...
      List<String> serverImages = selectedImages?.where((e) => e.serverUrl.isNotEmpty).map((e) => e.serverUrl).toList() ?? [];
      variationPostDataModel.images = serverImages;
      variationPostDataModel.productFilesExternal = selectedDigitalLinks?.where((test) => test.isSaved == false).toList() ?? [];
      ...
      if (widget.productVariationsID != null) {
        print("********************** Inside Update variation *************************");
        final updateProductVariationProvider = context.read<VendorCreateUpdateVariationViewModel>();
        final result = await updateProductVariationProvider.vendorUpdateProductVariation(context: context, variationID: widget.productVariationsID!, productPostDataModel: variationPostDataModel);
        if (result) {
          await _onRefresh();
        ...
        print("********************** Inside Create variation *************************");
        final createVariationProvider = context.read<VendorCreateUpdateVariationViewModel>();
        final result = await createVariationProvider.vendorCreateProductVariation(context: context, productID: widget.productID, productPostDataModel: variationPostDataModel);
        if (result) {
          context.read<VendorGetProductVariationsViewModel>().clearList();
          context.read<VendorGetProductVariationsViewModel>().vendorGetProductVariations(productID: widget.productID);
        }
  ...
  Widget _buildUi(BuildContext context) {
    return Consumer<VendorCreateProductViewModel>(
      builder: (context, provider, _) {
        ...
        final isDigital = provider.editVariationsApiResponse.data?.data?.product.productType.value?.toLowerCase() == VendorProductType.digital.name.toLowerCase();
        ...
        return Padding(
          padding: EdgeInsets.only(left: kPadding, right: kPadding, bottom: kLargePadding),
          child: Column(
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      _overviewSection(overview: overview, provider: provider),
  ...
  Widget _buildCreateVariationAttributes() {
    return Column(
      children: _selectedAttributesSet.map((attribute) {
        return Column(
          children: [
            fieldTitle(
            ...
            CustomDropdown(
                ...
                menuItemsList: attribute.attributes.map((elements) {
  ...
  Widget _buildAttributes({required List<ProductAttributeSet> list}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...list.map(
              ...
              CustomDropdown(
                ...
                menuItemsList: variation.attributes
                    .map((val) => DropdownMenuItem(
                          child: Text(val.title ?? ''), // Null safety check
                          value: val,
                        ))
                    .toList(),
                onChanged: (value) {
                  ...
                  if (value is EditAttributeData) {
                    ...
                    for (var item in listAttributes) {
                      for (var attr in item.attributes) {
                        print("Updated Attribute ID: ${attr.id}, isDefault: ${attr.isDefault}");
                      }
                    }
                    setState(() {});
                  }
  ...
  /// overview section
  Widget _overviewSection({required VariationProduct? overview, required VendorCreateProductViewModel provider}) {
    return Column(
      children: [
        TitleWithArrow(
          title: "Overview",
  ...
  _handleOverViewData(VendorProductOverviewModel? result) {
    // if (result != null) {
    overviewModel = result;
    variationPostDataModel.sku = overviewModel?.sku ?? context.read<VendorCreateProductViewModel>().generalSettingsApiResponse.data?.data?.sku?.toString() ?? '';
    ...
    variationPostDataModel.allowCheckoutWhenOutOfStock = overviewModel?.allowCustomerCheckoutWhenProductIsOutOfStock ?? false ? '1' : '0';
    variationPostDataModel.quantity = overviewModel?.withWareHouseManagement ?? false ? overviewModel?.quantity ?? '0' : '0';
  ...
  Widget _shippingSection({required BuildContext context}) {
    return Column(
      children: [
        TitleWithArrow(
          title: "Shipping",
  ...
  void _openDigitalPickerScreen() async {
    ...
    setState(() {
      ...
      for (var e in selectedDigitalImages ?? []) {
        print(e);
      }
      _updateDigitalImageCount();
    });
  ...
  showOverviewDetails({required VariationProduct? overview}) async {
    // overviewModel = VendorProductOverviewModel(
    //   sku: overview?.sku ?? '',
    //   price: overview?.price.toString() ?? '',
    //   priceSale: overview?.salePrice?.toString() ?? '',
