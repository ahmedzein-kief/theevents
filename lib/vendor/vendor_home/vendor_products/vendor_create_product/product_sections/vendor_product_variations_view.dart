...
class _VendorProductHasVariationsViewState extends State<VendorProductHasVariationsView> with MediaQueryMixin {
  ...
  Future _onRefresh() async {
    try {
      // setProcessing(true);
      final provider = Provider.of<VendorGetProductVariationsViewModel>(context, listen: false);

      /// clear list on refresh
      provider.clearList();
      setState(() {});
      await provider.vendorGetProductVariations(productID: widget.productID ?? '', searchString: _searchController.text);
                ...
                return Expanded(
                  child: VendorDataListBuilder(
                    ...
                    contentBuilder: (context) {
                      return _buildRecordsList(provider: provider);
                    },
                  ),
                );
              },
  ...
  Widget _buildRecordsList({required VendorGetProductVariationsViewModel provider}) {
          ...
          children: [
            CustomRecordListTile(
              ...
              subtitleAsWidget: Column(
                ...
                children: [
                  RichText(
                    text: TextSpan(
                      children: [
                        TextSpan(
                          text: "Quantity: ",
                          style: TextStyle(fontWeight: FontWeight.w500, fontSize: 12, color: AppColors.peachyPink),
                        ),
                        TextSpan(
                          text: productVariation.quantity?.toString().trim() == "&#8734;" ? "\u221E" : productVariation.quantity?.toString() ?? "--",
                          style: TextStyle(fontWeight: FontWeight.w500, fontSize: 13, color: Colors.black), // Add emphasis if needed
                        ),
                      ],
                    ),
                  ),
                  ...
                  VendorCustomRadioListTile(
                    ...
                    groupValue:
                        provider.list.any((p) => p.isDefault == true) ? provider.list.firstWhere((p) => p.isDefault ?? false, orElse: () => provider.list.first).isDefault : null,
                    // Set null if no item is default
                    padding: EdgeInsets.zero,
                    onChanged: (value) => _setDefaultVariation(productVariation: productVariation, provider: provider),
                    title: "Is Default",
              ...
              centerWidget:  Column(
                ...
                mainAxisSize: MainAxisSize.min,
                children: [
                  (productVariation.fileInternalCount?.toString() ?? '').isNotEmpty ? Row(
                    crossAxisAlignment: CrossAxisAlignment.center,
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text(
                  ...
                  (productVariation.fileExternalCount?.toString() ?? '').isNotEmpty ?  Row(
                    crossAxisAlignment: CrossAxisAlignment.center,
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text(
                      ...
                      Transform.rotate(
                        ...
                        child: Icon(Icons.link_outlined, color: Colors.black, size: 20),
                      ),
                    ],
                  ) : kShowVoid
                ],
          ...
          children: [
            CustomIconButtonWithText(
              ...
              onPressed: () async {
                ...
                final result = await vendorGetSelectedAttributesSetProvider.vendorGetSelectedProductAttributes(productID: widget.productID!);
                        ...
                        return SingleChildScrollView(
                          ...
                          child: Column(
                            children: [
                              ...
                              kFormFieldSpace,
                              Center(
                                child: Text("Select Attributes", style: TextStyle(fontSize: 20)),
                              ),
                              kFormFieldSpace,
                              ...
                              Padding(
                                ...
                                child: Wrap(
                                  ...
                                  children: attributesSet.map((attribute) {
                                    bool isSelected = _selectedAttributesSet.any((selected) => selected.id == attribute.id);
                                      ...
                                      onChanged: (value) {
                                        setModalState(() {
                                          if (value == true) {
                                            ...
                                            _selectedAttributesSet.removeWhere((selected) => selected.id == attribute.id);
                                          }
                                        });
                                      },
                              ...
                              if (_selectedAttributesSet.isEmpty)
                                Padding(
                                  padding: EdgeInsets.symmetric(horizontal: kPadding + kPadding),
                                  child: Align(
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  ChangeNotifierProvider(
                                    ...
                                    child: Consumer<VendorEditProductAttributesViewModel>(builder: (context, editAttributesProvider, _) {
                                      return SizedBox(
                                        ...
                                        child: CustomAppButton(
                                          ...
                                          onTap: () async {
                                            if (_selectedAttributesSet.length < 1) {
                                              AlertServices.showErrorSnackBar(message: "Please select at least 1 attribute.", context: context);
                                              Navigator.pop(context);
                                              return;
                                            }

                                            try {
                                              List<int> attributesIDsSet = [];

                                              /// add all attribute ids
                                              for (var attribute in _selectedAttributesSet) {
                                                attributesIDsSet.add(attribute.id);
                                              }
                                              final result = await editAttributesProvider.vendorEditProductAttributes(
                                                  productID: widget.productID, attributesSet: attributesIDsSet, context: context);
            ...
            ChangeNotifierProvider(
              ...
              child: Consumer<VendorGenerateAllProductVariationsViewModel>(builder: (context, provider, _) {
                    ...
                    deleteItemAlertDialog(
                        ...
                        onDelete: () async {
                          try {
                            Navigator.pop(context);
                            setProcessing(true);
                            final result = await provider.vendorGenerateAllProductVariations(productID: widget.productID, context: context);
  ...
  _onRowTap({required BuildContext context, required ProductVariationRecord rowData}) {
                      ...
                      child: SingleChildScrollView(
                        child: Column(
                          ...
                          children: [
                            ...
                            buildRow("Quantity", rowData.quantity?.toString().trim() == "&#8734;" ? "\u221E" : rowData.quantity?.toString() ?? "--"),
                            ListView.builder(
                                shrinkWrap: true,
                                physics: NeverScrollableScrollPhysics(),
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return buildRow(getAttributeName(entries?[index].key.toString() ?? ''), entries?[index].value.toString() ?? '');
                                }),
                          ],
                        ),
                      ),
  ...
  _onEditRecord({required ProductVariationRecord rowData}) async {
    await Navigator.push(
      context,
      CupertinoPageRoute(
        builder: (context) => VendorEditVariations(
  ...
  _onDeleteRecord({required ProductVariationRecord rowData}) async {
    deleteItemAlertDialog(
        context: context,
        onDelete: () async {
  ...
  _setDefaultVariation({required ProductVariationRecord productVariation, required VendorGetProductVariationsViewModel provider}) {
    ...
    if (!(productVariation.isDefault ?? false)) {
      deleteItemAlertDialog(
          ...
          customDescription: RichText(
            ...
            TextSpan(text: 'Do you want to set product variation with id', style: TextStyle(color: Colors.black, fontSize: 12)),
            TextSpan(text: ''' "${productVariation.id}" ''', style: TextStyle(color: AppColors.peachyPink, fontWeight: FontWeight.bold, fontSize: 12)),
            TextSpan(text: 'to default.', style: TextStyle(color: Colors.black, fontSize: 12))
          ])),
          buttonText: "Save",
          ...
          onDelete: () async {
            try {
              Navigator.pop(context);
              setProcessing(true);
              final result = await vendorSetDefaultProductVariationProvider.vendorSetDefaultProductVariation(productVariationID: productVariation.id.toString(), context: context);
              if (result) {
                setState(() {
                  provider.list.forEach((p) => p.isDefault = false);
  ...
  _setDeletionProcessing({required ProductVariationRecord rowData, required bool processing}) {
    setState(() {
  ...
  String getAttributeName(String attributeID) {
    try {
      final vendorGetAttributesSetProvider = context.read<VendorCreateProductViewModel>();
      final attributesSet = vendorGetAttributesSetProvider.attributeSetsApiResponse.data?.data ?? [];
      final attribute = attributesSet.firstWhere((attribute) {
        return attribute.id.toString() == attributeID.toString();
      });
