...
class UploadImagesScreen extends StatefulWidget {
  ...
  UploadImagesScreen({this.initialImages});
...
class _UploadImagesScreenState extends State<UploadImagesScreen> {
  List<UploadImagesModel> _selectedImages = [];
  ...
  Future uploadImages() async {
    try {
      ...
      final results = await provider.uploadAllImages(context, _selectedImages.where((file) => file.hasFile).toList());
      if (results.isNotEmpty) {
        print('Result 1 ==> ${results.length} ');
        for (var element in results) {
          if (element != null) {
            var matchedImage = _selectedImages.firstWhere(
  ...
  Future<void> _pickImage() async {
    ...
    List<UploadImagesModel> uploadImagesList = (myFiles ?? [])
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    if (!_selectedImages[index].hasFile) {
                      Uri uri = Uri.parse(_selectedImages[index].serverFullUrl);
