...
class _StoreViewState extends State<StoreView> {
  ...
  _initializeTheCountryDropdown() {
    countryMenuItemsList = countryModel?.data?.list?.map((element) {
  ...
  _initializeTheField({required VendorGetSettingsViewModel vendorGetSettingsProvider}) {
    ...
    if (storeInfo != null) {
      ...
      if (contentController.text.isNotEmpty) {
        _contentQuilController.document = Document.fromDelta(convertHtmlToDelta(htmlContent: contentController.text));
      }
    ...
    return SimpleCard(
      ...
      child: Column(
        children: [
          ...
          Material(
            child: CustomEditableTextField(placeholder: "Enter content here..", quillController: _contentQuilController,),
          ),
          kFormFieldSpace,
              ...
              return CustomAppButton(
                  ...
                  onTap: () async {
                    try {
                      ...
                      contentController.text = convertDeltaToHtml(quilController: _contentQuilController);
                      if (_formKey.currentState?.validate() ?? false) {
                        _createForm();
                        final result = await provider.vendorSettings(vendorSettingsType: VendorSettingType.store, form: form, context: context);
                        setProcessing(false);
  ...
  Widget _chooseImage({File? myFile, String? title, String? imageLink, required Function(File?) onFileSelected, required Function(File?) onFileClose}) {
    return Column(
      ...
      children: [
        ...
        Text(
          title ?? '',
          style: headingFields().copyWith(fontSize: 16, fontWeight: FontWeight.w500, color: Colors.black),
        ),
        const SizedBox(
          height: 8,
        ),
        ChooseFileCard(
          ...
          onChoose: () async {
            ...
            if (file != null) {
              onFileSelected(file); // Update the parent state with the selected file
            }
          },
          onClose: () {
            onFileClose(null);
        ...
        GestureDetector(
          ...
          child: Text(
            "Choose Image",
            style: headingFields().copyWith(fontSize: 16, fontWeight: FontWeight.w400, color: Colors.red),
          ),
        )
      ],
    );
  ...
  void _createForm() {
    form = FormData.fromMap({
      ...
      if (logoFile != null) "logo_input": MultipartFile.fromFileSync(logoFile!.path, filename: logoFile!.path.split('/').last), // Default placeholder if null
      if (coverImageFile != null) "cover_image_input": MultipartFile.fromFileSync(coverImageFile!.path, filename: coverImageFile!.path.split('/').last)
    });
  }
}
