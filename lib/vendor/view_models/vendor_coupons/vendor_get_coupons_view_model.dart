...
class VendorGetCouponsViewModel with ChangeNotifier {
  String? _token;
  int _currentPage = 1;
  int _lastPage = 1;

  final List<CouponRecords> _list = [];
  ...
  ApiResponse<VendorGetCouponsModel> _apiResponse = ApiResponse.none();

  ApiResponse<VendorGetCouponsModel> get apiResponse => _apiResponse;

  set setApiResponse(ApiResponse<VendorGetCouponsModel> response) {
    _apiResponse = response;
    notifyListeners();
  }

  /// get coupons method
  vendorGetCoupons({String? search}) async {
    await setToken();
    try {
      ...
      if (_currentPage <= _lastPage) {
        ...
        final VendorGetCouponsModel response = await _myRepo.vendorGetCoupons(headers: headers, queryParams: queryParams);
  ...
  void setLastPage(VendorGetCouponsModel response) {
    _lastPage = response.data?.pagination?.lastPage ?? 0;
    notifyListeners();
  }

  void resetList(VendorGetCouponsModel response) {
