...
class VendorEditProductAttributesViewModel with ChangeNotifier {
  ...
  Future<bool> vendorEditProductAttributes({required dynamic productID, required List<int> attributesSet, required BuildContext context}) async {
    try {
      setLoading(true);
      ...
      var body = jsonEncode({'attribute_sets': attributesSet});

      CommonPostRequestModel response = await _myRepo.vendorEditProductAttributes(headers: headers, productID: productID.toString(), body: body);
