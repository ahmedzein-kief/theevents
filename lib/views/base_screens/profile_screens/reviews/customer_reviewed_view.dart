...
class _CustomerReviewedViewState extends State<CustomerReviewedView> with MediaQueryMixin {
  ...
  Future _onRefresh() async {
    try {
      final provider = Provider.of<CustomerGetProductReviewsViewModel>(context, listen: false);
                  ...
                  children: [
                    VendorDataListBuilder(
                      onNoDataAvailable: Padding(
                        ...
                        child: Text('You have not reviewed any product.', style: GoogleFonts.inter(fontSize: 20, color: AppColors.peachyPink, fontWeight: FontWeight.bold), textAlign: TextAlign.start,),
                      ),
  ...
  Widget _buildRecordsList({required CustomerGetProductReviewsViewModel provider}) {
            ...
            children: [
              RecordListTile(
                ...
                subtitleAsWidget: Column(
                    ...
                    children: [
                      Text(record.createdAt.toString(),style:  dataColumnTextStyle(),),
                      Text(
                ...
                endWidget: Column(
                  ...
                  children: [
                    ...
                    GestureDetector(
                      ...
                      child: record.isDeleting ? SizedBox(height: 20,width: 20,child: Utils.pageLoadingIndicator(context: context)) : SvgPicture.asset("assets/vendor_assets/settings/delete_record.svg"),
                    )
                  ],
                ),
              ),
  ...
  _onRowTap({required BuildContext context, required ReviewedRecords rowData}) {
                      ...
                      child: SingleChildScrollView(
                        child: Column(
                          ...
                          children: [
                            Container(
                              height: 200,
                              decoration: BoxDecoration(image: DecorationImage(image: NetworkImage(rowData.image.toString()))),
                            ),
                            kSmallSpace,
                            ...
                            buildWidgetRow("Star", RatingBar.builder(
  ...
  _onDeleteRecord({required ReviewedRecords rowData}) async {
    deleteItemAlertDialog(
        context: context,
        onDelete: () async {
          _setDeletionProcessing(rowData: rowData, processing: true);
          Navigator.pop(context);
          final CustomerGetProductReviewsViewModel provider = Provider.of<CustomerGetProductReviewsViewModel>(context, listen: false);
          final result = await context.read<CustomerDeleteReviewViewModel>().customerDeleteReview(reviewID: rowData.id, context: context);
  ...
  _setDeletionProcessing({required ReviewedRecords rowData, required bool processing}) {
    setState(() {
