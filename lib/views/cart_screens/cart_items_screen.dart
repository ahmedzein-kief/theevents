...
import '../../utils/storage/image_picker.dart';
  ...
  Future<void> update(String rowId, int qty, List<Product>? products) async {
    ...
    if (products != null) {
      products.forEach((entry) {
        var cartRowId = entry.cartItem.rowId;
        var quantity = cartRowId == rowId ? qty.toString() : entry.cartItem.qty.toString();
              ...
              child: Column(children: [
                  ...
                  child: Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        CircleAvatar(
                          radius: 16.5,
                          backgroundImage: _selectedImage != null ? FileImage(_selectedImage!) : AssetImage("assets/boy.png") as ImageProvider,
                        ),
                      ],
                    ),
                  ),
                ...
                Expanded(
                  child: provider.cartLoading
                      ? Center(child: CircularProgressIndicator(color: Colors.black, strokeWidth: 0.5))
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            dynamic frontSalePrice = productCart?.product.prices.frontSalePrice;
                            dynamic price = productCart?.product.prices.price;
                            ...
                            return GestureDetector(
                              ...
                              child: ProductCartItems(
                                ...
                                quantity: 'Quantity: ${productCart?.cartItem.qty}',
                                brandDescription: "Sold By ${productCart?.product.store?.name}",
                                attributes: productCart?.cartItem.options.attributes ?? '',
                                ceoData: productCart?.cartItem.options.options,
                                offPrice: offPercentage.isNotEmpty ? '$offPercentage%off' : '',
                                actualPrice: 'AED ${productCart?.cartItem.price}',
                                standardPrice: (productCart?.product.prices.frontSalePrice ?? 0) < (productCart?.product.prices.price ?? 0)
                                    ? (productCart?.product.prices.priceWithTaxes ?? '')
                                    : '',
                                onSubtractPressed: () async {
                                  var products = provider.cartResponse?.data.products;
                                  var qty = productCart?.cartItem.qty;

                                  if ((qty ?? 0) > 0) {
                                    qty = (qty! - 1);
                                    await update(productCart!.cartItem.rowId, qty, products);
                                  }
                                },
                                onAddPressed: () async {
                                  var products = provider.cartResponse?.data.products;
                                  await update(productCart!.cartItem.rowId, (productCart.cartItem.qty + 1), products);
                                },
                                onDeletePressed: () async {
                                  await handleDelete(productCart?.cartItem.rowId ?? '');
                                },
                              ),
                            );
                          },
                ...
                Container(
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.only(
                      ...
                      topRight: Radius.circular(8), // Radius for the top-right corner
                    ),
                    boxShadow: [
                      BoxShadow(
                        color: Colors.black,
                  ...
                  child: Padding(
                    padding: EdgeInsets.only(right: screenWidth * 0.06, top: screenHeight * 0.02, bottom: screenHeight * 0.01),
                    child: Column(
                      ...
                      children: [
                        Padding(
                          ...
                          child: Column(mainAxisAlignment: MainAxisAlignment.start, crossAxisAlignment: CrossAxisAlignment.end, mainAxisSize: MainAxisSize.min, children: [
                            Text(AppStrings.subTotal + "${provider.cartResponse?.data.totalPrice}", style: cartSubtotal(context)),
                            Padding(
                                padding: EdgeInsets.symmetric(vertical: screenHeight * 0.02),
                                child: Text("${AppStrings.tax + "${provider.cartResponse?.data.formattedRawTax ?? ''}"}", style: cartSubtotal(context))),
                            Column(mainAxisAlignment: MainAxisAlignment.spaceAround, crossAxisAlignment: CrossAxisAlignment.end, mainAxisSize: MainAxisSize.max, children: [
                              Text(AppStrings.total + "${provider.cartResponse?.data.formattedFinalTotal}", style: cartTotal(context)),
                            ]),
                            Align(
                              alignment: Alignment.centerRight,
                              child: Text(AppStrings.shippingFees, textAlign: TextAlign.start, style: shippingFeesText(context)),
                            ),
                          ]),
                        ),
                        Padding(
                          ...
                          child: CustomAppButton(
                            ...
                            onTap: () async {
                              var checkoutToken = provider.cartResponse?.data.tracked_start_checkout;

                              if (checkoutToken != null) {
                                var response = await fetchCheckoutData(context, checkoutToken);
