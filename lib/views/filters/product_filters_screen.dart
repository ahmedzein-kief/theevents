            ...
            child: Row(
              children: [
                Expanded(
                  flex: 1,
                  child: ListView(
                    children: selectedIds.keys.map((filter) {
                      return ListTile(
                        title: Text(
                          filter,
                          style: TextStyle(
                  ...
                  child: selectedFilter == 'Prices'
                          ...
                          child: SfRangeSlider(
                            ...
                            onChanged: (SfRangeValues values) {
                              setState(() {
                                ...
                                selectedIds['Prices'] = [values.start.toInt(), values.end.toInt()];
                              });
                            },
                            enableTooltip: true,
                      ...
                      ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            var filterValue = getFilterValues()[index];
                            String value = filterValue['name'];
                            int id = filterValue['id'];
                            bool isSelected = selectedIds[selectedFilter]!.contains(id);

                            // Check if the current filter is a color and extract the color
                            Color? color;
                            if (selectedFilter == 'Colors') {
                              String colorName = filterValue['color'];
                                ...
                                children: [
                                  ...
                                  if (color != null)
                                    Container(
                                      width: 20,
                                      height: 20,
                                      margin: EdgeInsets.only(right: 8), // Margin between color and text
                                      decoration: BoxDecoration(
                                        color: color,
  ...
  List<Map<String, dynamic>> getFilterValues() {
    switch (selectedFilter) {
      ...
      case 'Colors':
        ...
        return colorAttributes.map((attribute) => {'name': attribute.title, 'id': attribute.id, 'color': attribute.color}).toList();
      default:
        return [];
    }
  }
