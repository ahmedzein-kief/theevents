...
class _UserTypeInnerPageScreenState extends State<UserTypeInnerPageScreen> {
  ...
  Future<void> fetchUserByTypePackages() async {
    try {
      ...
      await Provider.of<ProductProvider>(context, listen: false)
          .fetchPackages(storeId: userData!.storeId, context, perPage: 12, page: _currentPage, sortBy: CommonVariables.selectedSortBy);
      setState(() {
        _isFetchingMorePackages = false;
      });
    } catch (error) {
              ...
              Consumer<UsersByTypeProvider>(
                builder: (context, userTypeProvider, child) {
                  if (userTypeProvider.isLoading) {
                    ...
                    final userData = userTypeProvider.userData;
                    String? listingType = userData?.listingType;
                    // print("StoreId${userData!.storeId}");
                    return Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: <Widget>[
                        CustomSearchBar(hintText: "Search Events"),
                        Expanded(
                          child: SingleChildScrollView(
                            ...
                            child: Column(
                              children: [
                                          ...
                                          children: [
                                            Container(
                                              ...
                                              decoration: BoxDecoration(
                                                ...
                                                // Rounded corners
                                                border: Border.all(width: 2, color: Colors.white), // Optional: border for better visibility
                                              ),
                                              child: ClipRRect(
                                ...
                                ///======  TAB PAGES VIEW =================================
                                _currentTab == 'Products' ? _ProductsView(storeId: userData?.storeId.toString() ?? '') : _PackagesView(storeId: userData?.storeId.toString() ?? ''),
                              ],
                            ),
                          ),
                        ),
  ...
  Widget _PackagesView({required String storeId}) {
    ...
    return Column(
      children: [
        Consumer<ProductProvider>(builder: (context, packageProductProvider, child) {
          if ((packageProductProvider.isLoadingPackages && _currentPagePackages == 1)) {
            ...
            return Padding(
                ...
                child: Container(
                    width: double.infinity,
                    decoration: BoxDecoration(color: AppColors.lightCoral, borderRadius: BorderRadius.circular(5)),
                    height: 50,
                            ...
                            child: CustomPackagesView(
                              ...
                              onHeartTap: () async {
                                ...
                                if (isInWishlist) {
                                  ...
                                  await wishlistProvider.deleteWishlistItem(product.id, context, token ?? '');
                                } else {
                                  // Add to wishlist
