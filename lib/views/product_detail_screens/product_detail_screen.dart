...
class _ProductDetailScreenState extends State<ProductDetailScreen> {
  ...
  Future<void> fetchCustomerReviews() async {
    final provider = Provider.of<ProductItemsProvider>(context, listen: false);
    await provider.fetchCustomerReviews(provider.apiResponse?.data?.record?.id?.toString() ?? '', context);
  }

  Future<void> fetchItems() async {
    final provider = Provider.of<ProductItemsProvider>(context, listen: false);
    var result = await provider.fetchProductData(widget.slug, context);
    if (result != null) {
      final record = result.data?.record;
      if (record?.attributes?.isNotEmpty == true) {
        record?.attributes?.forEach(
          (newData) {
            var childOption = newData.children.firstWhere((childData) => childData.selected == true);

            if (childOption != null) {
              var attribute = selectedAttributes.firstWhere(
        ...
        var resultAttributes = await updateProductAttributes(selectedAttributes);
        if (resultAttributes != null) {
          updateProductAttributePrice = resultAttributes.data.price.toString() ?? '';
          productVariationId = resultAttributes.data.id;
          unavailableAttributes = resultAttributes.data.unavailableAttributeIds.map((e) => int.tryParse(e.toString()) ?? 0).toList();
        }
      }
    }
  }

  Future<ProductVariationModel?> updateProductAttributes(List<Map<String, dynamic>?> selectedAttributes) async {
    final provider = Provider.of<ProductItemsProvider>(context, listen: false);
    return await provider.updateProductAttributes(productID, context, selectedAttributes);
  }
            ...
            Padding(
              ...
              child: Consumer<ProductItemsProvider>(
                builder: (context, provider, child) {
                  ...
                  productPrice = isAttributesChanged ? updateProductAttributePrice : record?.prices?.frontSalePrice?.toString() ?? '';

                  if (record?.attributes?.isNotEmpty == true) {
                    record?.attributes?.forEach(
                      (newData) {
                        var childOption = newData.children.firstWhere((childData) => childData.selected == true, orElse: () => Child.defaultData());
                  ...
                  double? frontSalePrice = record.prices?.frontSalePrice?.toDouble();
                  double? price = record.prices?.price?.toDouble();
                            ...
                            children: [
                              ...
                              if (record.attributes?.isNotEmpty == true)
                                ProductAttributesScreen(
                                  ...
                                  onSelectedAttributes: (selectedAttribute) async {
                                    var result = await updateProductAttributes(selectedAttribute);
                                    if (result != null) {
                                      if (result.data.successMessage == null) {
                                        CustomSnackbar.showError(context, "Out of stock");
                                      }
                                      // Update selected attributes
                                      selectedAttribute.forEach((attr) {
                                        var matchingSelected = result.data.selectedAttributes.firstWhere(
                                          (selected) => selected.setId == attr?['attribute_set_id'],
                                          orElse: () => SelectedAttribute(setSlug: "", setId: -1, id: -1, slug: ""),
                                        );
                                      ...
                                      record.attributes?.forEach((newData) {
                                        ...
                                        newData.children.forEach((child) => child.selected = false);

                                        for (var updateAttr in selectedAttribute) {
                                          var matchingChild = newData.children.firstWhere(
                                      ...
                                      setState(() {
                                        ...
                                        unavailableAttributes = result.data.unavailableAttributeIds.map((e) => int.tryParse(e.toString()) ?? 0).toList();
                              ...
                              if (record.options.isNotEmpty == true)
                                ExtraProductOptions(
                                  ...
                                  screenWidth: screenWidth,
                                  selectedOptions: this.selectedExtraOptions,
                                  extraOptionsError: extraOptionErrorData,
                                  onSelectedOptions: (Map<String, dynamic> selectedExtraOptions) {
                                    setState(() {
                              ...
                              Padding(
                                ...
                                child: Column(
                                  children: [
                                    if (record.content.isNotEmpty) Text('Product Details', style: productValueItemsStyle(context)),
                                    Padding(
                              ...
                              Padding(
                                ...
                                child: Column(
                                  children: [
                                    Text('Customer Reviews', style: productValueItemsStyle(context)),
                                    Padding(padding: const EdgeInsets.only(top: 10), child: Container(height: 0.5, color: Colors.grey)),
                                    Padding(
                                        ...
                                        children: [
                                          if (provider.isReviewLoading) ...{
                                            ...
                                            if (record.review != null && provider.apiReviewsResponse != null) ...{
                                              ...
                                              Padding(padding: const EdgeInsets.only(top: 20), child: Container(height: 0.5, color: Colors.grey)),
                                            },
                                          },
                                          if (record.brand != null)
                                            Padding(
                                              ...
                                              child: Column(
                                                children: [
                                                  Text("ProductCode - 202t86876", style: productsReviewDescription(context)),
                                                  Text("Selling By ${record.brand!.name}", style: productsReviewDescription(context)),
                                                  Padding(
                                                    padding: const EdgeInsets.only(top: 10),
                                                    child: GestureDetector(
                                                      onTap: () {
                                                        Navigator.push(
                                                          context,
                                                          MaterialPageRoute(
                                                            builder: (context) => FeaturedBrandsItemsScreen(slug: record.brand?.slug ?? ''),
                                                          ),
                                                        );
                                                      },
                                                      child: Column(
                                                        children: [
                                                          Text("View ${record.brand!.name}->", style: viewShopText(context)),
                                                        ],
                                                      ),
                                                    ),
                                                  ),
